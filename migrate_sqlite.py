...
ensure_column(cur, "users", "nivel_acesso VARCHAR(20) NOT NULL DEFAULT 'UNIDADE'", "nivel_acesso")
...
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_users_nivel_acesso ON users (nivel_acesso)", "ix_users_nivel_acesso")
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_users_regional_id ON users (regional_id)", "ix_users_regional_id")
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_users_municipio_ibge ON users (municipio_ibge)", "ix_users_municipio_ibge")
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_unidades_regional_id ON unidades (regional_id)", "ix_unidades_regional_id")
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_unidades_municipio_ibge ON unidades (municipio_ibge)", "ix_unidades_municipio_ibge")
ensure_index(cur, "CREATE INDEX IF NOT EXISTS ix_pacientes_municipio_ibge ON pacientes (municipio_ibge)", "ix_pacientes_municipio_ibge")
