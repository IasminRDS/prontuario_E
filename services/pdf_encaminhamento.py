...
PRIORIDADE_LABELS = {
    'eletivo': 'ELETIVO', 'prioritario': 'PRIORITÁRIO', 'urgente': 'URGENTE'
}
...
def _estilos():
    ...
    return {
        'titulo':   ParagraphStyle('t',  **base, fontSize=13, fontName='Helvetica-Bold',
                                   textColor=AZUL, spaceAfter=2),
        'secao':    ParagraphStyle('s',  **base, fontSize=8,  fontName='Helvetica-Bold',
                                   textColor=AZUL2, spaceBefore=10, spaceAfter=4),
        'corpo':    ParagraphStyle('c',  **base, fontSize=9,  leading=14, spaceAfter=4),
        'label':    ParagraphStyle('l',  **base, fontSize=7.5, fontName='Helvetica-Bold',
                                   textColor=colors.HexColor('#5A6478')),
        'valor':    ParagraphStyle('v',  **base, fontSize=9,  leading=12),
        'rodape':   ParagraphStyle('r',  **base, fontSize=7,
        ...
        'prio':     ParagraphStyle('p',  fontName='Helvetica-Bold', fontSize=11,
...
def gerar_encaminhamento(enc, paciente, medico, unidade):
    ...
    ]], colWidths=[17.5*cm], rowHeights=[0.8*cm])
    tbl_prio.setStyle(TableStyle([
    ...
    dados_pac = [
        ...
        [Paragraph('Idade', e['label']),
         ...
         Paragraph('Masculino' if paciente.sexo=='M' else 'Feminino' if paciente.sexo=='F' else '—', e['valor'])],
    ...
    if paciente.alergias:
        dados_pac.append([Paragraph('⚠ Alergias', e['label']),
                          Paragraph(paciente.alergias, e['valor']), Paragraph('', e['label']), Paragraph('', e['valor'])])
    ...
    enc_dados = [
        ...
        [Paragraph('Serviço destino', e['label']),
         ...
         Paragraph(enc.data_agendada.strftime('%d/%m/%Y %H:%M') if enc.data_agendada else '—', e['valor'])],
    ]
    if enc.hipotese_diagnostica:
        enc_dados.append([Paragraph('Hipótese diag.', e['label']),
                          Paragraph(enc.hipotese_diagnostica, e['valor']),
                          Paragraph('', e['label']), Paragraph('', e['valor'])])
    ...
    s.append(Paragraph(
        ...
        f'{datetime.now().strftime("%d de %B de %Y")}', e['centro']))
    ...
    s.append(Paragraph(
        ...
        f'Encaminhamento #{enc.id}  ·  Sistema de Prontuário Único SUS', e['rodape']))
