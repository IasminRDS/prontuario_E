...
def _estilos():
    ...
    return {
        'titulo':    ParagraphStyle('titulo',    **base, fontSize=13, fontName='Helvetica-Bold',
                                    textColor=AZUL_SUS,  spaceAfter=2),
        'subtitulo': ParagraphStyle('subtitulo', **base, fontSize=9,  textColor=colors.HexColor('#5A6478'),
                                    spaceAfter=8),
        'secao':     ParagraphStyle('secao',     **base, fontSize=8,  fontName='Helvetica-Bold',
                                    textColor=AZUL_MEDIO, spaceBefore=10, spaceAfter=4,
                                    borderPad=2),
        'corpo':     ParagraphStyle('corpo',     **base, fontSize=9,  leading=14, spaceAfter=4),
        'label':     ParagraphStyle('label',     **base, fontSize=7.5, fontName='Helvetica-Bold',
                                    textColor=colors.HexColor('#5A6478')),
        'valor':     ParagraphStyle('valor',     **base, fontSize=9,  leading=12),
        'rodape':    ParagraphStyle('rodape',    **base, fontSize=7,  textColor=colors.HexColor('#8A95A8'),
                                    alignment=TA_CENTER),
...
def _cabecalho(story, unidade, titulo_doc, e):
    ...
    cab = Table([
        [Paragraph('<b><font color="#FFFFFF">SUS</font></b>',
                   ParagraphStyle('sus', fontSize=9, fontName='Helvetica-Bold',
                                  backColor=AZUL_SUS, textColor=colors.white,
                                  borderPad=4, alignment=TA_CENTER)),
         Paragraph(f'<b>{unidade.nome if unidade else "Unidade de Saúde"}</b><br/>'
...
def _campo(label, valor, e):
    return [Paragraph(label, e['label']), Paragraph(str(valor) if valor else '—', e['valor'])]
...
def gerar_prontuario(prontuario, paciente, medico, unidade):
    ...
    dados = [
        ['Nome', paciente.nome_exibicao, 'Data de Nasc.', paciente.data_nascimento.strftime('%d/%m/%Y')],
        ['CNS', paciente.cns or '—', 'CPF', paciente.cpf or '—'],
        ['Idade', f'{paciente.idade} anos', 'Sexo',
         'Masculino' if paciente.sexo == 'M' else 'Feminino' if paciente.sexo == 'F' else '—'],
        ['Mãe', paciente.nome_mae or '—', 'Tipo Sanguíneo', paciente.tipo_sanguineo or '—'],
        ['Município', f'{paciente.municipio or "—"}/{paciente.uf or ""}', 'Telefone', paciente.telefone or '—'],
    ]
    if paciente.alergias:
        dados.append(['⚠ Alergias', paciente.alergias, '', ''])

    tbl = Table([[Paragraph(c if i % 2 == 0 else str(c), e['label'] if i % 2 == 0 else e['valor'])
                  for i, c in enumerate(row)] for row in dados],
    ...
    # Sinais vitais
    vitais = []
    if prontuario.pressao_arterial: vitais.append(('PA', prontuario.pressao_arterial, 'mmHg'))
    if prontuario.temperatura:      vitais.append(('Temp.', f'{prontuario.temperatura:.1f}', '°C'))
    if prontuario.frequencia_cardiaca: vitais.append(('FC', str(prontuario.frequencia_cardiaca), 'bpm'))
    if prontuario.saturacao_o2:     vitais.append(('SpO₂', f'{prontuario.saturacao_o2:.0f}', '%'))
    if prontuario.peso:             vitais.append(('Peso', f'{prontuario.peso:.1f}', 'kg'))
    if prontuario.altura:           vitais.append(('Altura', f'{prontuario.altura:.2f}', 'm'))
    if prontuario.imc:              vitais.append(('IMC', f'{prontuario.imc:.1f}', 'kg/m²'))
    if prontuario.glicemia:         vitais.append(('Glicemia', str(int(prontuario.glicemia)), 'mg/dL'))

    if vitais:
        story.append(Paragraph('SINAIS VITAIS', e['secao']))
        vrow = [[Paragraph(f'<b>{v[0]}</b><br/><font size="10">{v[1]}</font><br/>'
                           f'<font size="7" color="#8A95A8">{v[2]}</font>', e['assinatura'])
                 for v in vitais]]
    ...
    if prontuario.prescricao:
        ...
        story.append(Paragraph(prontuario.prescricao.replace('\n', '<br/>'), e['corpo']))

    # Encaminhamento
    if prontuario.encaminhamento:
        story.append(Paragraph('ENCAMINHAMENTOS', e['secao']))
        story.append(Paragraph(prontuario.encaminhamento.replace('\n', '<br/>'), e['corpo']))
    ...
    if prontuario.assinado_em:
        story.append(Paragraph(
            f'Assinado em {prontuario.assinado_em.strftime("%d/%m/%Y às %H:%M")}', e['assinatura']))
...
def gerar_receituario(prontuario, paciente, medico, unidade):
    ...
    if prontuario.retorno_dias:
        story.append(Paragraph(
            f'<b>Retorno:</b> em {prontuario.retorno_dias} dias.', e['corpo']))

    # Assinatura
    story.append(Spacer(1, 2*cm))
    story.append(HRFlowable(width='8*cm', thickness=0.5, color=CINZA_BORDA,
    ...
    story.append(Paragraph(
        ...
        f'{datetime.now().strftime("%d de %B de %Y")}', e['assinatura']))
...
def gerar_atestado(paciente, medico, unidade, dias, cid=None, observacao=None):
    ...
    story.append(Paragraph(texto,
                           ParagraphStyle('at', fontName='Helvetica', fontSize=10,
                                          leading=18, textColor=CINZA_TEXTO, spaceAfter=12)))

    if cid:
        story.append(Paragraph(f'CID-10: <b>{cid}</b>',
                               ParagraphStyle('cid', fontName='Helvetica', fontSize=9,
                                              textColor=CINZA_TEXTO, spaceAfter=8)))
    if observacao:
        story.append(Paragraph(f'Observações: {observacao}',
                               ParagraphStyle('obs', fontName='Helvetica', fontSize=9,
                                              textColor=CINZA_TEXTO, spaceAfter=8)))
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle('data', fontName='Helvetica', fontSize=9,
                       textColor=CINZA_TEXTO, alignment=TA_RIGHT, spaceAfter=30)))
...
def _extenso(n):
    nums = {1:'um',2:'dois',3:'três',4:'quatro',5:'cinco',6:'seis',7:'sete',
