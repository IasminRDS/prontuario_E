...
def _estilos():
    return {
        'titulo':  ParagraphStyle('ti', fontName='Helvetica-Bold',   fontSize=14, textColor=AZUL,   spaceAfter=2),
        'sub':     ParagraphStyle('su', fontName='Helvetica',         fontSize=9,  textColor=CINZA,  spaceAfter=2),
        'secao':   ParagraphStyle('se', fontName='Helvetica-Bold',    fontSize=8,  textColor=AZUL2,  spaceBefore=10, spaceAfter=4, textTransform='uppercase'),
        'corpo':   ParagraphStyle('co', fontName='Helvetica',         fontSize=9,  leading=14, textColor=CINZA, spaceAfter=4),
        'corpo_j': ParagraphStyle('cj', fontName='Helvetica',         fontSize=9,  leading=14, textColor=CINZA, alignment=TA_JUSTIFY),
        'label':   ParagraphStyle('la', fontName='Helvetica-Bold',    fontSize=7.5,textColor=colors.HexColor('#5A6478')),
        'valor':   ParagraphStyle('va', fontName='Helvetica',         fontSize=9,  leading=12, textColor=CINZA),
        'rodape':  ParagraphStyle('ro', fontName='Helvetica',         fontSize=7,  textColor=colors.HexColor('#8A95A8'), alignment=TA_CENTER),
        'centro':  ParagraphStyle('ce', fontName='Helvetica',         fontSize=9,  alignment=TA_CENTER, textColor=CINZA),
        'bold':    ParagraphStyle('bo', fontName='Helvetica-Bold',    fontSize=9,  textColor=CINZA),
    }
...
def _tabela_info(dados, e, col_widths=None):
    ...
    for i in range(0, len(dados), 2):
        ...
        for j in range(2):
            if i + j < len(dados):
                lbl, val = dados[i + j]
                row.extend([Paragraph(lbl, e['label']), Paragraph(str(val) if val else '—', e['valor'])])
            else:
...
def gerar_alta(internacao, paciente, medico, unidade):
    ...
    cab = Table([[
        Paragraph('<b><font color="#FFFFFF">SUS</font></b>',
                  ParagraphStyle('sus', fontSize=9, fontName='Helvetica-Bold',
                                 backColor=AZUL, textColor=colors.white,
        ...
        Paragraph(datetime.now().strftime('%d/%m/%Y'),
                  ParagraphStyle('d', fontName='Helvetica', fontSize=8,
                                 alignment=TA_RIGHT, textColor=CINZA)),
    ...
    tipo_alta_map = {
        'curado': 'Alta — Curado', 'melhorado': 'Alta — Melhorado',
        'transferencia': 'Transferência', 'obito': 'Óbito',
        'a_pedido': 'Alta a pedido', 'evasao': 'Evasão',
    }
    tipo_alta_lbl = tipo_alta_map.get(internacao.tipo_alta or '', internacao.tipo_alta or '—')

    s.append(_tabela_info([
        ('Data de entrada',   internacao.data_entrada.strftime('%d/%m/%Y')),
        ('Data de alta',      internacao.data_alta.strftime('%d/%m/%Y') if internacao.data_alta else '—'),
    ...
    if internacao.sumario_alta:
        s.append(KeepTogether([
            ...
            Paragraph(internacao.sumario_alta.replace('\n', '<br/>'), e['corpo_j']),
        ]))
        s.append(Spacer(1, 0.2*cm))
    ...
    if prescricoes_ativas:
        ...
        for i, item in enumerate(pres.itens, 1):
            ...
            if item.dose:        partes.append(item.dose)
            if item.via:         partes.append(item.via)
            if item.frequencia:  partes.append(item.frequencia)
            if item.duracao:     partes.append(f'por {item.duracao}')
    ...
    if cirurgias:
        ...
        for c in cirurgias:
            ...
            if c.data_inicio: linha += f' — {c.data_inicio.strftime("%d/%m/%Y")}'
            if c.duracao_real: linha += f' ({c.duracao_real} min)'
    ...
    exames = ExameSolicitado.query.filter_by(
        internacao_id=internacao.id if hasattr(internacao, 'id') else None
    ).all() if hasattr(internacao, 'id') else []
    ...
    if exames:
        ...
        for ex in exames[:10]:
            status_map = {'resultado':'Com resultado','pendente':'Pendente','cancelado':'Cancelado'}
    ...
    assin = Table([[
        ...
        Table([[
            Paragraph('_' * 40, e['centro']),
            Paragraph('Assinatura / Digital do paciente ou responsável', e['centro']),
        ]], colWidths=[8.75*cm]),
