...
def ocupacao():
    ...
    return render_template('relatorios_hosp/ocupacao.html',
                           setores=setores, total=total, ocupados=ocupados,
                           taxa=taxa, internacoes_mes=internacoes_mes,
                           permanencia_media=permanencia_media, altas=altas)
...
def producao():
    ...
    try:
        ...
        di = datetime.now().replace(day=1); df = datetime.now()

    uid = current_user.unidade_id
    internacoes = Internacao.query.filter(Internacao.data_entrada.between(di, df), Internacao.unidade_id==uid).count()
    altas       = Internacao.query.filter(Internacao.data_alta.between(di, df), Internacao.unidade_id==uid, Internacao.status=='alta').count()
    cirurgias   = Cirurgia.query.filter(Cirurgia.data_agendada.between(di, df), Cirurgia.unidade_id==uid).count()
    cir_realizadas = Cirurgia.query.filter(Cirurgia.data_agendada.between(di, df), Cirurgia.unidade_id==uid, Cirurgia.status=='realizada').count()
    ps_total    = AtendimentoPS.query.filter(AtendimentoPS.data_entrada.between(di, df), AtendimentoPS.unidade_id==uid).count()
    ps_internados = AtendimentoPS.query.filter(AtendimentoPS.data_entrada.between(di, df), AtendimentoPS.unidade_id==uid, AtendimentoPS.desfecho=='internado').count()
    ...
    return render_template('relatorios_hosp/producao.html',
...
def relatorio_ps():
    ...
    if exportar == 'csv':
        buf = StringIO()
        w = csv.writer(buf)
        w.writerow(['Data entrada','Paciente','Classificação','Modo chegada','Status','Desfecho','Tempo (min)'])
        for a in ats:
            w.writerow([a.data_entrada.strftime('%d/%m/%Y %H:%M'),
                        a.paciente.nome_exibicao, a.classificacao,
                        a.modo_chegada, a.status,
                        a.desfecho or '', a.tempo_total_min or ''])
        buf.seek(0)
        return send_file(BytesIO(buf.getvalue().encode('utf-8-sig')),
                         mimetype='text/csv', as_attachment=True,
                         download_name=f'relatorio_ps_{date.today()}.csv')

    return render_template('relatorios_hosp/ps.html',
                           ats=ats, por_cor=por_cor, por_desfecho=por_desfecho,
