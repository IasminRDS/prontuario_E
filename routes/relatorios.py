...
def pacientes():
    ...
    if sexo:      q = q.filter(Paciente.sexo == sexo)
    if municipio: q = q.filter(Paciente.municipio.ilike(f'%{municipio}%'))
    if tem_cns == '1': q = q.filter(Paciente.cns.isnot(None), Paciente.cns != '')
    elif tem_cns == '0': q = q.filter(db.or_(Paciente.cns.is_(None), Paciente.cns == ''))
    if idade_min:
    ...
    return render_template('relatorios/pacientes.html',
                           pacientes=pacientes,
                           filtros=dict(sexo=sexo, municipio=municipio,
...
def _csv_pacientes(pacientes):
    buf = StringIO()
    w = csv.writer(buf)
    w.writerow(['Nome','Nome Social','CNS','CPF','Data Nasc.','Idade','Sexo',
                'Raça/Cor','Telefone','Município','UF','Alergias'])
    for p in pacientes:
        w.writerow([p.nome, p.nome_social or '', p.cns or '', p.cpf or '',
                    p.data_nascimento.strftime('%d/%m/%Y'), p.idade,
                    p.sexo, p.raca_cor or '', p.telefone or '',
                    p.municipio or '', p.uf or '', p.alergias or ''])
    buf.seek(0)
...
def atendimentos():
    ...
    if tipo: q = q.filter(Atendimento.tipo == tipo)
    ...
    if exportar == 'csv':
        ...
        for a in ats:
            w.writerow([a.data_hora.strftime('%d/%m/%Y %H:%M'),
                        a.paciente.nome_exibicao, a.tipo, a.status,
    ...
    return render_template('relatorios/atendimentos.html',
                           atendimentos=ats, totais_tipo=totais_tipo,
...
def producao():
    ...
    total_ag   = Agendamento.query.filter(Agendamento.data_hora.between(di, df), Agendamento.unidade_id == uid).count()
    total_tri  = Triagem.query.filter(Triagem.criado_em.between(di, df), Triagem.unidade_id == uid).count()
    total_exam = ExameSolicitado.query.filter(ExameSolicitado.data_solicitacao.between(di, df), ExameSolicitado.unidade_id == uid).count()
    total_enc  = Encaminhamento.query.filter(Encaminhamento.data_solicitacao.between(di, df), Encaminhamento.unidade_origem_id == uid).count()
    total_vac  = VacinaAplicada.query.filter(VacinaAplicada.criado_em.between(di, df), VacinaAplicada.unidade_id == uid).count()
    ...
    faltas = Agendamento.query.filter(
        ...
        Agendamento.unidade_id == uid).count()
    ...
    if exportar == 'csv':
        ...
        for label, val in [
            ('Atendimentos', total_at), ('Prontuários', total_pront),
            ('Agendamentos', total_ag), ('Triagens', total_tri),
            ('Exames solicitados', total_exam), ('Encaminhamentos', total_enc),
            ('Vacinas aplicadas', total_vac), ('Novos pacientes', novos_pac),
    ...
    return render_template('relatorios/producao.html',
                           ...
                           total_at=total_at, total_pront=total_pront,
                           total_ag=total_ag, total_tri=total_tri,
                           total_exam=total_exam, total_enc=total_enc,
                           total_vac=total_vac, novos_pac=novos_pac,
                           faltas=faltas)
...
def triagem():
    ...
    ).order_by(Triagem.criado_em.desc()).all()
    ...
    if exportar == 'csv':
        buf = StringIO()
        w   = csv.writer(buf)
        w.writerow(['Data','Paciente','Classificação','Queixa','PA','Temp.','SpO2','Dor'])
        for t in triagens:
            w.writerow([t.criado_em.strftime('%d/%m/%Y %H:%M'),
                        t.paciente.nome_exibicao, t.classificacao,
                        t.queixa_principal or '', t.pressao_arterial or '',
                        t.temperatura or '', t.saturacao_o2 or '',
    ...
    return render_template('relatorios/triagem.html',
                           triagens=triagens, por_cor=por_cor,
                           data_ini=data_ini, data_fim=data_fim)
