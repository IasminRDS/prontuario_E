...
def index():
    ...
    triagens = (Triagem.query
                .filter(db.func.date(Triagem.criado_em) == hoje,
...
def nova(paciente_id=None):
    ...
    if request.method == 'POST':
        try:
            ...
            t = Triagem(
                ...
                glicemia         = float(request.form['glicemia']) if request.form.get('glicemia') else None,
                ...
                dor_escala       = int(request.form['dor_escala']) if request.form.get('dor_escala') else None,
            ...
            db.session.add(t)
            db.session.flush()
            registrar('triagens', t.id, 'create',
                      f'Triagem {t.classificacao} — paciente {t.paciente_id}')
            db.session.commit()
    ...
    return render_template('triagem/form.html',
                           pacientes=pacientes,
...
def visualizar(id):
    ...
    if t.discriminadores:
        try:
            ...
            pass
    return render_template('triagem/visualizar.html',
                           triagem=t, discriminadores=discriminadores)
...
def atualizar_status(id):
    ...
    if novo in ('aguardando', 'em_atendimento', 'finalizado'):
        t.status = novo
        registrar('triagens', t.id, 'update', f'Status triagem → {novo}')
        db.session.commit()
    return redirect(url_for('triagem.index'))
...
def historico_paciente(paciente_id):
    ...
    triagens = (Triagem.query
                ...
                .all())
    return render_template('triagem/historico.html',
                           paciente=paciente, triagens=triagens)
