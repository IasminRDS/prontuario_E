...
def toggle_usuario(id):
    ...
    registrar('users', user.id, acao, f'Usuário {"ativado" if user.ativo else "desativado"}')
    db.session.commit()
...
def auditoria():
    ...
    q = AuditLog.query
    if tabela:  q = q.filter(AuditLog.tabela == tabela)
    if acao:    q = q.filter(AuditLog.acao == acao)
    if usuario:
        u = User.query.filter(User.nome.ilike(f'%{usuario}%')).first()
        if u: q = q.filter(AuditLog.usuario_id == u.id)
    ...
    return render_template('admin/auditoria.html', logs=logs,
                           ...
                           filtro_tabela=tabela, filtro_acao=acao, filtro_usuario=usuario)
