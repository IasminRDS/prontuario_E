...
class ItemEstoque(db.Model):
    ...
    unidade         = db.relationship('Unidade',    backref='itens_estoque')
    medicamento     = db.relationship('Medicamento', backref='item_estoque')
    movimentacoes   = db.relationship('MovEstoque',  backref='item',
                                      lazy='dynamic', cascade='all, delete-orphan')

    @property
    def status(self):
        if self.quantidade <= 0:                         return ('Zerado',  'vermelho')
        if self.quantidade <= self.estoque_minimo:       return ('Crítico', 'vermelho')
        if self.quantidade <= self.estoque_minimo * 1.5: return ('Baixo',   'amarelo')
        return ('Normal', 'verde')
...
class MovEstoque(db.Model):
    ...
    unidade      = db.relationship('Unidade', backref='movimentacoes_estoque')
    ...
    @property
    def tipo_label(self):
        return self.TIPO_LABELS.get(self.tipo, (self.tipo, 'cinza'))
